...
Nodepins.prototype.init = function() {
	...
	self.loadToken(function() {
		self.loadImageData(function() {
			console.log(BUF.length + " bytes in InageData Buffer");
		});
...
Nodepins.prototype.loadImageData = function(callback) {
	...
	fs.readFile('./lib/images.json','utf8',function(err, content) {
		...
		BUF = new Buffer(content, encoding='utf8');
		//buf.len = buf.length;		
		// 		//buf = new Buffer(imageDataJson, encoding='utf8');
		//sys.log(buf.length + "bytes in buf");
...
Nodepins.prototype.serverExtensions = function() {
	...
	self.serverAuth ={
		incoming: function(message,callback) {
			...
			//console.log("INCOMING: " + message.clientId + " Message Channel: " + message.channel);
			if (message.channel === ('/meta/handshake' || '/meta/connect'))
			...
			if (message.channel === '/meta/subscribe') {
				var subscription = message.subscription,
					msgToken	 = message.ext.authToken || "false";
		...
		//console.log("OUTGOING: " + " clientID: " + message.clientId + " Channel: " + message.channel);
		if (message.channel === ('/meta/subscribe' || '/meta/handshake'))
			return callback(message);
		    
		//if (message.chennel === '/meta/connect')
		
		if (message.channel === '/meta/connect' && self.subscriber[message.clientId]) {
				...
				if (!message.extInt) {
					...
					message.extInt.bigData = BUF.toString('utf8', 0, BUF.length);
			...
			//for (var key in self.subscriber) {
				// var tmpsubscr = self.subscriber[message.clientId];
				var ms = (Date.now()) - (self.subscriber[message.clientId]).timestamp;
				sys.log("Subscriber: " + message.clientId + " online since : " + Math.round((ms/1000/60)) + " Min");
			//}
