...
Nodepins.prototype.serverExtensions = function() {
	...
	self.serverAuth ={
		incoming: function(message,callback) {
			...
			console.log(self.objSize(self.subscriber));
			for (var key in self.subscriber) {
				console.log("hello" + self.subscriber[key]);
			}
			...
			var subscription = message.subscription,
				...
				if (!self.subscriber[message.clientId])
				 	{
				 	self.subscriber[message.clientId] = (Date.now());
				 	}
